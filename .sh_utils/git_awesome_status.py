...
from sh_utils import cmd, blue_str, grey_str, red_str, argpop, get_term_size, \
    bold_str, magenta_str
...
def clean_up_decorate(lines):
    ...
    for line in lines.split('\n'):
        ...
        if m:
            decorate = m.group(1)
            tags = decorate.split(',')
            tags = [t.strip() for t in tags if not t.strip().startswith(
                'deploy_build') and not t.strip().startswith('deployed')]
            if not tags:
...
def show_sha(sha):
    res = clean_up_decorate(cmd(
        'git --no-pager log --color=always --pretty=format:"%Cgreen%H%Creset   %an %C(yellow)%d%Creset%n   %s%n" -1 ' + sha))
    res = res.replace("Joseph Zhong", blue_str("Joseph Zhong"))
    print(res)
...
def show_shas(sha_first, sha_last, nmax=100):
    res = clean_up_decorate(cmd(
        'git --no-pager log -n {} --color=always --pretty=format:"%Cgreen%H%Creset   %an %C(yellow)%d%Creset%n   %s%n" {}...{}'.format(
            nmax, sha_first, sha_last)))
    res = res.replace("Joseph Zhong", blue_str("Joseph Zhong"))
    print(res)
    return res
...
def show_my_most_recent(fb):
    print('               ....\n')
    res = clean_up_decorate(cmd(
        'git --no-pager log --color=always --author=srussell --pretty=format:"%Cgreen%H%Creset   %an %C(yellow)%d%Creset%n   %s%n" -1 ' + fb))
    res = res.replace("Joseph Zhong", blue_str("Joseph Zhong"))
    print(res)
...
def show_sha_grey(sha):
    res = cmd(
        'git --no-pager log --pretty=format:"%H   %an%n   %s%n%Creset" -1 ' + sha)
    if "Joseph Zhong" in res:
        ts = res.split("Joseph Zhong")
        print(grey_str(ts[0]) + blue_str("Joseph Zhong") + grey_str(ts[1]))
    else:
        print(grey_str(res))
    print('')
...
def show_sha_magenta(sha):
    res = cmd(
        'git --no-pager log --pretty=format:"%C(magenta)%H%Creset   %an%n   %s%n" -1 ' + sha)
    res = res.replace("Joseph Zhong", blue_str("Joseph Zhong"))
    print(res)
    print('')
...
while True:
    ...
    made_merged = [c for c in tot[0:50] if
                   c.title in origintitle and c.sha not in originsha]
    missing = [c for c in origin[0:(tot_n - len(made))] if
               c.sha not in totsha and c.title not in tottitle]
...
for branch in other_branches:
    try:
        stats = cmd(
            'git show --pretty=format:"%ci %cr" ' + branch + ' -- | head -n 1').split(
            ' ')
        dt = datetime.datetime.strptime(stats[0] + ' ' + stats[1],
                                        "%Y-%m-%d %H:%M:%S")
        age = stats[3] + ' ' + stats[4]
    except:
...
for idx, dat in enumerate(
        sorted(branch_data, key=itemgetter(1), reverse=True)):
    branch, dt, age = dat
...
###########################################################
if 'Joseph Zhong' not in originz and 'Joseph Zhong' not in originz:
